...
import { distance } from "@/islands/articles/flow-fields-methods/distance.tsx";
...
export function CollisionDetectionIllustration() {
  ...
  const createLine = (x: number, y: number) => {
    ...
    while (x > 0 && y > 0 && x < maxWidth && y < maxHeight) {
      ...
      if (
        points.some(
          ([px, py, pr]) => distance([x, y], [px, py]) < (r + pr) / 2 + 5,
        )
      ) {
        break;
      }
    ...
    <Illustration>
      <canvas
        ...
        onTouchMove={(event) => {
          ...
          createLine(touch.clientX - bbox.left, touch.clientY - bbox.top);
        }}
      />
      <div className="pt-2">
        <Button
